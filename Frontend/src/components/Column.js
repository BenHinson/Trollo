...
export default function Column({ id, columnName, deleteColumn }) {
  const [taskData, setTaskData] = useState([
    { columnId: 1, name: "Take out the trash", description: "Quickly!", userId: 1 },
    { columnId: 2, name: "Brush teeth", description: "Properly!", userId: 2 },
    { columnId: 3, name: "Do the dishes", description: "Sparkling clean!", userId: 3 },
  ]);

  //TODO: Prop drilling userData for now, nasty practice, but it'll get solved with API calls
  const [userData, setUserData] = useState({
   1: { id: 1, name: "Ben", avatar: "https://www.winhelponline.com/blog/wp-content/uploads/2017/12/user.png"},
   2: { id: 2, name: "Jenny", avatar: "https://upload.wikimedia.org/wikipedia/commons/thumb/5/59/User-avatar.svg/1024px-User-avatar.svg.png"},
   3: { id: 3, name: "Gabriel", avatar: "https://cdn.icon-icons.com/icons2/1378/PNG/512/avatardefault_92824.png"}
  })
  ...
  const tasks = taskData
    ...
    .map((task) => {
      return <Task name={task.name} description={task.description} userData={userData} userId={task.userId}/>;
    });
