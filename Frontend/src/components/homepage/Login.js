...
export default function Login({ setAccountLogin, setDisplayModal }) {
    ...
    return (
        <div>
            ...
            <button className='navBtn submitAccountBtn' onClick={handleLogIn}>Login</button>
                    
            <button className='navBtn navBtnInv switchFormBtn' onClick={() => {setAccountLogin(false)}}>Create Account</button>
        </div>
    )
}