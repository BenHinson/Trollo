...
app.get("/projects", auth.middle, async (req, res) => {
  // Receives UID and returns names of projects and their id.
  let memberOfProjects = await ProjectMembers.findAll({
    where: { userId: req.uID },
  });
  memberOfProjects = memberOfProjects.map((e) => e.dataValues.projectId);

  Project.findAll({ where: { id: memberOfProjects } })
    .then((e) => {
      return res.json({
        message: "success",
...
app.get(
  ...
  checkUserIsMember,
  async (req, res) => {
    // Receives projectId. Returns Boards within.
    ...
    try {
      ...
      let members = (
        await ProjectMembers.findAll({ where: { projectId } })
      ).map((e) => e.dataValues.userId);
      data.members = (await User.findAll({ where: { id: members } })).map(
        (e) => {
          return {
            id: e.dataValues.id,
      ...
      data.boards = (await Board.findAll({ where: { projectId } })).map(
        (e) => e.dataValues
      );
...
app.get(
  ...
  checkUserIsMember,
  async (req, res) => {
    // Receives boardId. Returns columns. and their tasks.
    ...
    if (!returnData.board) {
      return res
        .status(400)
        .json({ error: `No such board with id: ${boardId}` });
    }

    let columnIds = [];
    (await Column.findAll({ where: { boardId: boardId } })).forEach((board) => {
      returnData.columns[board.dataValues.id] = {
        ...board.dataValues,
        ...{ tasks: [] },
      };
...
app.post("/project", auth.middle, async (req, res) => {
  // Create Project. Takes: {name}
  const uid = req.uID;
  const { name } = req.body;
...
app.post(
  ...
  checkUserIsMember,
  async (req, res) => {
    // Create Board for the assigned projectID.
    const { projectId } = req.params;
    const { name } = req.body;
    if (!name) {
      return res
        .status(400)
        .json({ error: "Please provide a name for the board" });
    }

    const background = ["#fafaeb", "#f3e4f1", "#d5ebda", "#f4cacd", "#ead3d4"][
...
app.post(
  "/project/:projectId/board/:boardId/column",
  auth.middle,
  checkUserIsMember,
  async (req, res) => {
    // Create Column from a name
    const { projectId, boardId } = req.params;
    const { name } = req.body;
...
app.post(
  "/project/:projectId/board/:boardId/column/:columnId/task",
  auth.middle,
  checkUserIsMember,
  async (req, res) => {
    // Create Task
    ...
    try {
      ...
      let newTask = await column.createTask({
        name,
        description,
        assigned,
        creatorId: req.uID,
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!newUser) {
        return res
          .status(400)
          .json({ error: "No Such User with that Email exists" });
      }
      if (
        (
          await ProjectMembers.findOne({
            where: { projectId, userId: req.uID },
          })
        )?.dataValues
      ) {
        return res
          .status(400)
          .json({ error: "The user is already a member of the project" });
      }
...
app.delete(
  ...
  checkUserIsAdmin,
  async (req, res) => {
    // Delete a specific board. Only if admin
    const { projectId, boardId } = req.params;
...
app.delete(
  "/project/:projectId/board/:boardId/column/:columnId",
  auth.middle,
  checkUserIsMember,
  async (req, res) => {
    // Delete a specific column
    const { projectId, boardId, columnId } = req.params;
    Column.destroy({ where: { id: columnId } })
      .then((e) => {
        return res.json({
          message: "success",
        });
...
app.patch(
  ...
  async (req, res) => {
    ...
    Column.update({ name }, { where: { id: columnId } })
      .then((e) => {
        res.json({
          message: "success",
          data: { name },
...
app.use((req, res) => {
  return res.json({ Status: "The API is working" });
});
...
async function checkUserIsMember(req, res, next) {
  return (
    await ProjectMembers.findOne({
      where: { projectId: req.params.projectId, userId: req.uID },
    })
  )?.dataValues
    ? next()
...
async function checkUserIsAdmin(req, res, next) {
  return (
    await Project.findOne({
      where: { id: req.params.projectId, adminId: req.uID },
    })
  )?.dataValues
    ? next()
